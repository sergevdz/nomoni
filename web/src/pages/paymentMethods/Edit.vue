<template>
  <q-page>
    <div class="q-pa-sm panel-header">
      <div class="row">
        <div class="col-sm-9">
          <span class="q-ml-md grey-8 fs28 page-title">Edit Type {{ $route.params.id }}</span>
        </div>
        <div class="col-sm-3">
      ...
      <div class="row bg-white content-panel">
        <div class="col q-pa-md">
          ...
          <div class="row q-col-gutter-xs">
            <div class="col-sm-4">
              <q-input
                v-model="paymentMethod.fields.name"
            ...
            <div class="col-sm-4">
              <q-input
                v-model="paymentMethod.fields.icon"
            ...
            <div class="col-sm-4">
              <q-input
                v-model="paymentMethod.fields.ord"
...
export default {
  ...
  created () {
    ...
    api.get(`/payment-methods/${id}`).then(({ data }) => {
      this.paymentMethod.fields = data.paymentMethod
      this.paymentMethod.fields.ord = `${data.paymentMethod.ord}`
    })
  },
  mounted () {},
  methods: {
    editType () {
      ...
      this.$q.dialog({
        title: 'Confirm',
        message: 'Do you want to edit the payment method?',
        cancel: true,
        persistent: true
      }).onOk(() => {
        const params = { ...this.paymentMethod.fields }
        api.put(`/payment-methods/${params.id}`, params).then(({ data }) => {
          this.$q.dialog({
